...
//    List<Book> findByBookName(String bookName);
    @Query(value = " SELECT * FROM book inner join category on book.category_id=category.category_id WHERE book_name LIKE %?1% OR category.`category_name` LIKE %?1% OR book_id LIKE %?1%" ,nativeQuery = true)
    List<Book> findByBookName(String bookName);
